<template>
	<view class="order">
		<uv-sticky bgColor="#fff">
			<view class="top">
				<uv-search searchIcon="search" placeholder="请输入企业名称" v-model="company" @search="search"
					@clear="search"></uv-search>
				<uv-tabs :list="tabsList" :scrollable="false" @click="tabSwitch" />
			</view>
		</uv-sticky>
		<view class="order_card">
			<view class="card_item" v-for="(item, index) in list" :key="index" @click="goDetail(item)">
				<view class="c_top">
					<uv-image src="https://cdn.uviewui.com/uview/album/1.jpg" width="48rpx" height="48rpx"></uv-image>
	...
	export default {
		...
		methods: {
			...
			goDetail({
				...
				if (status != 0) {
					uni.navigateTo({
						url: `/pages/order/orderDetail?id=${id}&productName=${productName}`
					})
				}
			},
			/** 切换tab */
