<template>
  <view class="container-list-item" @click="clickItem($event, item)">
    <label class="content">
      <view class="left-aaa" @click.stop="clickBox($event, item)">
        <!-- 复选框 -->
        <view class="checkbox" v-if="isCheckBox">
          <i
            v-if="curChecked"
            :style="{ color: checkActiveColor }"
            class="iconfont icon-xuanzhong txt icon-selected"
          />
          <i
            v-else
            :style="{ color: checkNoneColor }"
            class="iconfont icon-weixuanzhong txt"
          />
        </view>
        <!-- 复选框 -->
        <!-- 单选框 -->
        <view class="checkbox" v-else-if="isRadio">
          <i
            v-if="curChecked"
            :style="{ color: checkActiveColor }"
      ...
      <!-- 自定义插槽body -->
      <view v-if="hasChildren" class="right"
...
export default {
  ...
  data() {
    return {
      ...
      sChildren: this.comparison.children
        ? this.comparison.children
        : 'children', // children值名称
  ...
  computed: {
    ...
    hasChildren: function () {
      return Boolean(this.item)
        ? Boolean(this.item[this.sChildren])
          ? this.item[this.sChildren].length > 0
          : false
        : false
    },
    /**
    ...
    isRadio: function () {
      return (
        this.isCheck && !this.multiple && (!this.nodes || !this.hasChildren)
      )
    },
    /**
     * 是否多选
  ...
  watch: {
    ...
    comparison: {
      handler: function (val) {
        ...
        this.sChildren = this.comparison.children
          ? this.comparison.children
          : 'children'
      },
      deep: true
